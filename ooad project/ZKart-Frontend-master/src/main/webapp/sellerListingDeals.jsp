...
<html lang="en">
<head>
<title>Add Items to deals</title>
      			...
      			<div class="col-md-2"></div>
      				<div class="col-md-6">
      			    	...
      			    	<h4 class="unicase-checkout-title">
      				        ...
      				        href="#collapseUser">All Existing Deals</a>
      				     </h4>
      			    	</div>
      			    	<!-- panel-heading -->
            ...
            <div class="col-md-2"></div>
              <div class="col-md-6">
                ...
                <div class="panel-heading">
                  <h4 class="unicase-checkout-title">
                      <a data-toggle="collapse" class="" data-parent="#accordion"
                      href="#collapseUser">Items On Deals</a>
                   </h4>
                  </div>
                  <!-- panel-heading -->
                 ...
                 <div class="sidebar-widget wow fadeInUp" style="visibility: hidden; animation-name: none;">
                    <div class="row">
                      <div class="widget-header" style="margin-bottom:10px; margin-left:15px;">
                        <h3 class="widget-title"  style="color:#FF0000;"><B>ADD ITEMS TO EXISTING DEALS</B></h3>
                      </div>
                    </div>
                    <div class="row">
           	              ...
           	              <h4 class="widget-title"><B>ITEMS<label style="color:red; padding-left:3px;">  *</label></B></h4>
           	            </div>
              ...
              <div class="search-result-container"  style="text-align:center;">
                  <div class="sidebar-widget wow fadeInUp" style="visibility: hidden; animation-name: none;">
                    <div class="widget-header">
                      ...
                      <input type="button" onclick="addItemtoDeal()" style="font-size:18px;color: white;
		...
		success : function(deals_json){
            ...
            for(var i in deals_json){
               data+="<tr><td>"+deals_json[i].description+"</td>"+
				"<td>"+deals_json[i].discount+"</td></tr>";
            }
...
function fetchSellerItemsOnDeals()
{
  var sellerId = JSON.parse(getCookie("seller_details")).id;
	$.ajax(
	{
		type : 'GET',
		contentType : 'application/json',
		url : ctxPath + "/dealItems/seller/"+sellerId,
		dataType : "json", // data type of response
		success : function(deals_json){
			var data="";
			console.log(deals_json);
            for(var i in deals_json){
               data+="<tr><td>"+deals_json[i].item.listingName+"</td>"+
                    "<td>"+deals_json[i].deal.description+"</td>"+
				           "<td>"+deals_json[i].deal.discount+"</td></tr>";
            }
            $("#dealTable_body1").html(data);
    	},
    	error:function() {
        	swal("error occurred");
    	}
...
function addListDeal(ldJSON){
	$.ajax({
		type : 'POST',
		contentType : 'application/json',
		url : ctxPath + "/listingDeals/create",
		data : ldJSON,
		dataType : "json", // data type of response
		success : function(){
...
$("#submitListingDeal").click(function()
{
	...
	if(selectedDeal==="buy3get1"){
		...
		for(var num=8; num<10; num++){
			...
			//alert(listingDealJSON);
			addListDeal(listingDealJSON);categoryName
		}
	}
	else if(selectedDeal==="15%cashback"){
		...
		$.ajax({
			type : 'PUT',
			contentType : 'application/json',
			url :  ctxPath +"/item/update/"+listingid,
...
function setDeals()
{
  $.ajax(
  {
    ...
    dataType : "json", // data type of response
    success : function(result){
      var data="<select id='catId' name='catId' style=\" background-color:white; font-size:20px;\">"+"<option value=' "+0+" '>Select a Deal</option>";
            for(var i in result){
...
function setItems()
{
    var result = JSON.parse(getCookie("seller_details"));
    console.log(result);
    ...
    $.ajax(
    {
      type : 'GET',
      contentType : 'application/json',
      url : ctxPath + "/item/seller/"+itemId,
      dataType : "json", // data type of response
      success : function(result){
        var data="<select id='subcatId'  name ='subcatId' style=\" background-color:white; font-size:20px;\">"+"<option value=' "+0+" '>Select an Item</option>";
            for(var i in result){
