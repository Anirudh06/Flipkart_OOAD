           ...
           <div class="sidebar-widget wow fadeInUp" style="visibility: hidden; animation-name: none;">
              <div class="row">
	              <div class="widget-header" style="margin-bottom:10px; margin-left:15px;">
	                <h3 class="widget-title"  style="color:#FF0000;"><B>LIST OF ACCOUNTS</B></h3>
	                <hr/>
...
function viewAllAccounts() {
	...
	$.ajax({
		type : 'GET',
		url : ctxPath + "/accounts/user/all/" + user.id,
...
function render(op) {
	var tableData = "<table style='width:80%; font-size:20px'>" + 
    		"<tr> <th>Account Number</th><th colspan=1>Balance</th></tr>";
	if(op) {
		for(i = 0; i < op.length; i++) {
			tableData += "<tr><td>" + op[i].accountNumber + "</td>" 
					+ "<td>" + op[i].balance + "</td></tr>";
			}
	}
	tableData += "</table>";
...
function updateBalanceNow(){
	...
	console.log(ctxPath);
	var path = ctxPath + "/accounts/user/" + JSON.parse(getCookie("user_details")).id;
	console.log(path);
	$.ajax(
			{
				type : 'GET',
		    ...
		    success : function(data) {
			    ...
			    if (typeof data === 'undefined')
			    	...
			    	$.ajax({
			    	            type : 'PUT',
			    	            contentType : 'application/json',
			    	            url : ctxPath + "/accounts/addmoney/",
			    	            data : updateAccount(cct),
			    	            success : function(){
			    	            	$.ajax({type : 'GET', contentType : 'application/json',
			    	            				url : ctxPath + "/accounts/accountno/"+ cct.accountNumber,
			    	            				success : function(result){
			    	            		              swal("Updated balance is: " + result.balance);
			    	            		    	}
			    	            			});
			    					//window.location=(ctxPath + "/zkartAccountAddMoney.jsp");
			    	            }});
			    }});
}
...
function updateAccount(result) 
{
  ...
  var zkartAccount = JSON.stringify({
	  "accountNumber":result.accountNumber,
	  "balance":balance,
      "pin": pin
    });
  console.log(zkartAccount);
  console.log("here");
  return zkartAccount;
