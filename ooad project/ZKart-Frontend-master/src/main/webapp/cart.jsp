...
function renderCartItem(cartItems){
	...
	for(var i in cartItems){
		...
		$.ajax(
          ...
          var amount = Math.round(item.price - (item.discount*item.price/100));
						if(birthday == 1)
              amount = Math.round(amount - (item.bdaydiscount*amount/100));

            if(cartItems[i].deal != null)
                amount = Math.round(amount - (cartItems[i].deal.discount*amount/100));
				...
				"<td class='cart-product-name-info'>"+
					...
					"<span id='itemId" + i + "' hidden='hidden'>" + item.itemId + "</span>" +
	            ...
	            $("#content").append(data);
	            if(cartItems[i].deal != null) {
		            console.log(cartItems[i].deal.description + " " + i);
		            populateDealName(cartItems[i].deal.description, i);
	            }
		        getsellerid(item.user.id, i);
			  },
			  error:function() {
			  	swal("error occurred");
			  }
...
function getBirthdayDiscount(itemId) {
	var birthdayDiscount = 0;
	$.ajax({
		type : 'GET',
		data : 'json',
		async : false,
		...
		success : function(data) {
			console.log(data);
			if(data != null)
				birthdayDiscount = data.bdaydiscount;
		}
	});
...
function calculate(){
    ...
    // 	$("#deliveryCharges").text("50");
    // }
    $("#deliveryCharges").text("(" + num + " * 50)");
    $("#actualcarttotal").val(actualtotal);
	$("#amount-payable").text(parseInt($("#totalPrice").text())+ parseInt(num) * 50);
	$("#savings").text(savings);
}
...
function populateDealName(dealName, i) {
	document.getElementById("dealName"+i).innerHTML = "DEAL : "+ dealName;
	$("#dealName"+i).css("color : green;");
}
</script>
</body>
</html>
