...
function checkCookie()
{
	...
	function loadListings(){
	   ...
	   var sellerid = $("#sellerid").val();
		        			...
		        			success : function(listings_json)
		        			{
		        				for (var i in listings_json)
		        				{
									$.ajax(
									{
		        							type : 'GET',
		        							contentType : 'application/json',
		        							url : ctxPath + "/orders/item/"+listings_json[i].itemId,
		        							dataType : "json", // data type of response
		...
		function changestatus(orderid, i, j,orderObj){
		      console.log("changeStatus");
		      console.log("orderid");
		      orderObj=JSON.stringify(orderObj);
		      console.log(orderObj);
		      var obj=JSON.parse(orderObj);
		      obj.orderStatus="ORDER_SHIPPED";
		      var orderObj=JSON.stringify(obj);
		      console.log(orderObj);
			...
			$.ajax(
					...
					success : function()
					{
						 document.getElementById(i).innerHTML = "Item Status :  <b>Order Shipped </b>";
						 document.getElementById(j).style.visibility = "hidden";
						 swal("You Confirmed : Order Shipped.");
					},
					error:function(){
		        	    		swal("error occurred");
		        	    	}
      ...
      console.log(order_json.orderStatus);
			var data="";
			var i = $("#count").val();
			var j = $("#count2").val();
			j++;
	        ...
	        "<div  id='"+i+"' style = 'padding-bottom:10px;font-size:18px;color:green;'>Item Status :  <b>"+order_json.orderStatus+"</b></div><div><input  id='"+j+"' type='button'onclick='changestatus("+order_json.orderId+","+i+","+j+","+orderObj+");' value ='Goods Shipped'/></div>"+
	       	"</div></div>";


	       	$('#listing').append(data);
			if(order_json.orderStatus != "PAYMENT_RECEIVED" ){
				document.getElementById(j).style.visibility = "hidden";
			}
