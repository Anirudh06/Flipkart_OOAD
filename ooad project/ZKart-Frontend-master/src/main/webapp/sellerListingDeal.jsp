        ...
        <div class="search-result-container ">
           <div class="sidebar-widget wow fadeInUp" style="visibility: hidden; animation-name: none;">
		     ...
		     <div class="row">
	          ...
	          <div id="xoff">
	          	<div class="row">
	          		<div id="xoff-dealDesc" class="alert alert-warning" style="width: 550px;"></div>
	          		<input type="number" id="xoff-days" hidden="hidden"/>
	          	</div>
	          	<div class="row">
	      
  					<h4 class="widget-title"><B>ADD OFFER(%)<label style="color:red; padding-left:3px;">  *</label>:</B> <input type="text" id="xdynamic"></input></h4> 
  				
		          	<div class="col-sm-6 sidebar"> 
		              <h4 class="widget-title"><B>LISTING<label style="color:red; padding-left:3px;">  *</label></B></h4>
		              <div id="selectlisting-12"></div>
		            </div>
		        </div>
	          </div>
	       </div>
...
{
	var stDate1 = new Date();
	stDate1 = new Date(stDate1.getTime() - (stDate1.getTimezoneOffset() * 60000)).toJSON().slice(0,10);
	console.log(stDate1);
		            ...
		            for(var i in result){
		               ...
		               if(dealname==="buy3get1"){
		            	   ...
		            	   $("#buy3get1-days").val(result[i].days);
			           		...
			           		$("#xoff-dealDesc").html(result[i].description+"<br/>"+"Deal will be valid for "+result[i].days+" days starting today");
			           		$("#xoff-days").val(result[i].days);
			           	}
		            }
		            data += "</select>";
...
function addListDeal(ldJSON){
	$.ajax({
		type : 'POST',
		contentType : 'application/json',
		url : ctxPath + "/listingDealsNew/create",
		data : ldJSON,
		dataType : "json", // data type of response
		success : function(){
...
$("#submitListingDeal").click(function() 
{
	...
	if(selectedDeal==="buy3get1"){
		...
		combodesc = document.getElementById("buy1get1-combodesc").value;
   		...
   		days = document.getElementById("xoff-days").value;
   		off = document.getElementById("xdynamic").value;
   	}
	var stDate = new Date();
	...
	if(selectedDeal==="buy3get1"){
		...
		$.ajax({
			type : 'PUT',
			contentType : 'application/json',
			url :  ctxPath +"/listings/update/"+listingid,
		...
		var listingid = document.getElementById("list12").value;
		var listingDealJSON = JSON.stringify({
		    "dealid" : dealid,
		    "listingid" : listingid,
		    "startdate" : stDate,
		...
		$.ajax({
			...
			contentType : 'application/json',
			url :  ctxPath +"/listings/update/"+listingid,
			data : JSON.stringify({ "discount": off }),
			success : function()
			{
				//window.location.reload(true);
			},
