							...
							<div class="panel panel-default">
							    ...
							    <div class="">
									<ul id="orderProgress" class="nav nav-checkout-progress list-unstyled">
										...
										<li><a href="#" hidden="hidden" id="payStatus" class="progress-class"><b id="paymentStatus">Payment Received</b></a></li>
									</ul>
								</div>
							</div>
						</div>
...
function fetchCombo(){
	...
	$.ajax({
				...
				url : ctxPath + "/listingDealsNew/combo/"+comboid,
	...
	$.ajax(
			{
				type : 'GET',
				contentType : 'application/json',
				url : ctxPath + "/item/"+combo_json.item.id,
				success : function(listing_json)
				{
					if(listing_json.quantity>0)
					{
							...
							$.ajax(
							{
								type : 'GET',
								contentType : 'application/json',
								url : ctxPath + "/users/"+listing_json.user.id,
		...
		else
		{
			if($(".orderPlaced")[0])
			{
				...
				if(accountno=="" || pin=="" || isNaN(pin))
				{
				    ...
				    alert("pin length should be 4 and integer!!");
				    console.log("pin length incorrect !");
				    return false;
				  }
				else
				{	
...
console.log("hum bhi agye ="+amt);
console.log("hum bhi agye id laker ="+id);
	$.ajax(
		{
			type : 'PUT',
			contentType : 'application/json',
